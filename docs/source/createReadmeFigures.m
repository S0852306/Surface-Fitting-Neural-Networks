...
fitPlot = plot(x, yHat, 'r-', 'LineWidth', 1.8); hold on;
dataPlot = plot(x, y, 'k.', 'MarkerSize', 12);
grid on;
xlabel('x'); ylabel('y');
title(sprintf('Sharp square-wave-like 1D fit, MAE %.2e', bsplineModel.MeanAbsoluteError));
legend([dataPlot, fitPlot], {'Data', 'NN fit'}, 'Location', 'northeast');
...
dataPlot = scatter3(data(1,:), data(2,:), label(1,:), 18, 'k', 'filled');
view(35, 24); grid on;
xlabel('x'); ylabel('y'); zlabel('y1');
title('NN fit - out1');
legend([dataPlot, fitPlot], {'Data', 'NN fit'}, 'Location', 'best');

nexttile;
fitPlot = surf(xGrid, yGrid, output2Hat, 'EdgeColor', 'none'); hold on;
dataPlot = scatter3(data(1,:), data(2,:), label(2,:), 18, 'k', 'filled');
view(35, 24); grid on;
